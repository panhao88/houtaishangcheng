      ...
      <div style="margin-top:30px">
        ...
        <el-dialog title="分配用户" :visible.sync="dialogFormVisible2">
          <el-form :model="gototi" ref="ruleForm">
            <el-form-item label="当前用户:" label-width="100px">{{gototi.username}}</el-form-item>
            <el-form-item label="当前角色:" label-width="100px">{{gototi.role_name}}</el-form-item>
            <el-form-item label-width="100px">
              <el-select placeholder="请选择" clearable v-model="roleName1">
                <el-option
                  v-for="item in users"
                  :key="item.id"
                  :label="item.roleDesc"
          ...
          <div slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="querenqqq">确 定</el-button>
          </div>
        </el-dialog>
      </div>
    </el-card>
...
const {mapState:jurisdictionStste,mapActions:jurisdictionActions} = userModule
export default {
  name: "",
  //接收父组件传递过来的参数
  props: {},
  ...
  methods: {
    ...
    //确认分配用户
    querenqqq() {
      this.dialogFormVisible2 = false;r
      ...
      console.log(didi, this.gototi.id);
      this.getusersolp({
        id: this.gototi.id,
        rid: didi,
